...
const hrDashboardController = (socket, io) => {
  ...
  const validateEmployeeData = (data) => {
    ...
    if (typeof data.account.role !== 'string' || data.account.role.trim() === '') {
  ...
  socket.on('hrm/employees/get-employee-stats', async (payload) => {
    try {
      ...
      const departmentId = typeof rawDeparmentId === 'string' ? rawDeparmentId.trim() : '';
  ...
  socket.on('hrm/employees/get-employee-grid-stats', async (payload) => {
    try {
      ...
      const designationId = typeof rawdesignationId === 'string' ? rawdesignationId.trim() : '';
  ...
  socket.on(
    ...
    withRateLimit(async (payload) => {
      try {
        ...
        const policyName = typeof payload.policyName === 'string' ? payload.policyName.trim() : '';
  ...
  socket.on(
    ...
    withRateLimit(async (data) => {
      try {
        ...
        const policyName = typeof data.policyName === 'string' ? data.policyName.trim() : '';
    ...
    withRateLimit(async (data) => {
      try {
        ...
        const departmentName =
          typeof data.departmentName === 'string' ? data.departmentName.trim() : '';
        if (!departmentName) {
  ...
  socket.on(
    ...
    withRateLimit(async (data) => {
      try {
        ...
        const departmentId = typeof data._id === 'string' && data._id.trim() ? data._id.trim() : '';
  ...
  socket.on(
    ...
    withRateLimit(async (data) => {
      try {
        ...
        const result = await hrmDepartment.reassignAndDeleteDepartment(companyId, hrId, data);
    ...
    withRateLimit(async (data) => {
      try {
        ...
        const designationName =
          typeof data.designationName === 'string' ? data.designationName.trim() : '';
        if (!designationName) {
          throw new Error('Department name and display name are required');
        }
  ...
  socket.on('hrm/designations/get', async (filters) => {
    try {
      ...
      if (filters && typeof filters === 'object') {
        ...
        } else if (typeof filters.department === 'string' && filters.department.trim() !== '') {
  ...
  socket.on(
    ...
    withRateLimit(async (data) => {
      try {
        ...
        const result = await hrmDesignation.reassignAndDeleteDesignation(companyId, hrId, data);
  ...
  socket.on(
    ...
    withRateLimit(async (data) => {
      try {
        ...
        const response = await hrmEmployee.checkPhoneExists(companyId, phone, excludeEmployeeId);
  ...
  socket.on(
    ...
    withRateLimit(async (data) => {
      try {
        ...
        const response = await hrmEmployee.checkEmployeeLifecycleStatus(companyId, data.employeeId);
  ...
  socket.on(
    ...
    withRateLimit(async (data) => {
      try {
        ...
        const response = await hrmEmployee.updatePersonalInfo(companyId, hrId, data);
