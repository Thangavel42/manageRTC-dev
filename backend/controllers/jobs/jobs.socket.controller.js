...
const jobsSocketController = (socket, io) => {
  devLog("Setting up jobs socket controller...");

  const isDevelopment = process.env.NODE_ENV === "development" || process.env.NODE_ENV === "production";

  const validateAccess = (socket) => {
    if (!socket.companyId) {
      devError("[Jobs] Company ID not found in user metadata", {
  ...
  const handleGetJobsStats = withRateLimit(async (data) => {
    try {
      devLog("[Jobs] Getting jobs stats:", data);
      ...
      devError("[Jobs] Error getting jobs stats:", error);
  ...
  const handleGetJobsList = withRateLimit(async (data) => {
    try {
      ...
      devError("[Jobs] Error getting jobs list:", error);
  ...
  const handleGetJobDetails = withRateLimit(async (data) => {
    try {
      devLog("[Jobs] Getting job details:", data);
      const { companyId } = validateAccess(socket);
      const { jobId } = data;
      ...
      devError("[Jobs] Error getting job details:", error);
  ...
  const handleCreateJob = withRateLimit(async (data) => {
    try {
      devLog("[Jobs][CREATE] Incoming payload keys:", Object.keys(data || {}));
      if (data?.image) {
        devLog("[Jobs][CREATE] Image provided, dataURL length:", (data.image || '').length);
        devLog("[Jobs][CREATE] Image meta:", data.imageName, data.imageMime);
      }
      ...
      devLog("[Jobs][CREATE] Calling service with sanitized jobData (no image logged)");
      const result = await createJobService(jobData);
      devLog("[Jobs][CREATE] Service result:", { done: result?.done, message: result?.message });
      ...
      devError("[Jobs][CREATE] Error:", error);
  ...
  const handleUpdateJob = withRateLimit(async (data) => {
    try {
      ...
      devError("[Jobs] Error updating job:", error);
  ...
  const handleDeleteJob = withRateLimit(async (data) => {
    try {
      ...
      devError("[Jobs] Error deleting job:", error);
  ...
  const handleBulkDeleteJobs = withRateLimit(async (data) => {
    try {
      devLog("[Jobs] Bulk deleting jobs:", data);
      const { companyId, userId } = validateAccess(socket);
      const { jobIds } = data;
      ...
      devError("[Jobs] Error bulk deleting jobs:", error);
  ...
  const handleUpdateJobStatus = withRateLimit(async (data) => {
    try {
      devLog("[Jobs] Updating job status:", data);
      const { companyId, userId } = validateAccess(socket);
      ...
      devError("[Jobs] Error updating job status:", error);
  ...
  const handleExportJobsPDF = withRateLimit(async (data) => {
    try {
      devLog("[Jobs] Export PDF request:", data);
      const { companyId, userId } = validateAccess(socket);
        ...
        setTimeout(() => {
          try {
            if (result.data.filePath && fs.existsSync(result.data.filePath)) {
              ...
              devLog("[Jobs] Cleaned up PDF file:", result.data.filePath);
            }
          } catch (cleanupError) {
            devError("[Jobs] Error cleaning up PDF file:", cleanupError);
          }
        }, 60 * 60 * 1000); // 1 hour
      }
      ...
      devError("[Jobs] Error exporting PDF:", error);
  ...
  const handleExportJobsExcel = withRateLimit(async (data) => {
    try {
      devLog("[Jobs] Export Excel request:", data);
      const { companyId, userId } = validateAccess(socket);
      ...
      if (result.done) {
        ...
        setTimeout(() => {
          try {
            if (result.data.filePath && fs.existsSync(result.data.filePath)) {
              fs.unlinkSync(result.data.filePath);
              devLog("[Jobs] Cleaned up Excel file:", result.data.filePath);
            }
          } catch (cleanupError) {
            devError("[Jobs] Error cleaning up Excel file:", cleanupError);
          }
        }, 60 * 60 * 1000); // 1 hour
      }
      ...
      devError("[Jobs] Error exporting Excel:", error);
  ...
  devLog("Jobs socket controller setup complete");
};
