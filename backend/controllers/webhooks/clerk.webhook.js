...
function verifyWebhookSignature(req) {
  ...
  if (!webhookSecret) {
    devWarn('[Clerk Webhook] No webhook secret configured, skipping verification');
...
export const handleClerkWebhook = asyncHandler(async (req, res) => {
  devLog('[Clerk Webhook] Received webhook event');
  ...
  devLog('[Clerk Webhook] Event type:', type);
  devLog('[Clerk Webhook] User ID:', data?.id);
  ...
  switch (type) {
    ...
    default:
      devLog('[Clerk Webhook] Unhandled event type:', type);
  }
...
async function handleUserCreated(clerkUser) {
  devLog('[Clerk Webhook] Creating employee record for user:', clerkUser.id);
  ...
  try {
    ...
    if (!companyId) {
      devLog('[Clerk Webhook] No companyId in metadata, skipping employee creation');
      return;
    }
    ...
    if (existingEmployee) {
      devLog('[Clerk Webhook] Employee already exists for user:', clerkUser.id);
      return;
    }
    ...
    if (!email) {
      devError('[Clerk Webhook] No email found for user:', clerkUser.id);
      return;
    }
    ...
    if (emailExists) {
      devLog('[Clerk Webhook] Email already exists in employees, linking to Clerk user');
    ...
    // Create employee record (using canonical schema structure)
    ...
    if (result.insertedId) {
      devLog('[Clerk Webhook] Employee record created:', result.insertedId);
    } else {
      devError('[Clerk Webhook] Failed to create employee record');
    }
  } catch (error) {
    devError('[Clerk Webhook] Error creating employee record:', error);
...
async function handleUserUpdated(clerkUser) {
  devLog('[Clerk Webhook] Updating employee record for user:', clerkUser.id);
  ...
  try {
    ...
    if (!employee) {
      devLog('[Clerk Webhook] Employee not found for user:', clerkUser.id);
      return;
    }

    // Update employee with latest data (using canonical schema structure)
    const updateData = {
    ...
    devLog('[Clerk Webhook] Employee record updated:', employee._id);
  } catch (error) {
    devError('[Clerk Webhook] Error updating employee record:', error);
  }
}
...
async function handleUserDeleted(clerkUser) {
  devLog('[Clerk Webhook] Soft deleting employee record for user:', clerkUser.id);

  try {
    // We need to find the employee across all companies
    // For now, we'll skip this as it requires searching across all collections
    devLog('[Clerk Webhook] User deletion handling not implemented');
  } catch (error) {
    devError('[Clerk Webhook] Error deleting employee record:', error);
  }
}
